...
public class QuoteGeneratorServiceImpl implements QuoteGeneratorService{
	...
	public Flux<Quote> fetchQuoteStream(Duration period) {
		...
		//We are using flux here to create quotes, itarating on each stock starting from index 0
		return Flux.generate(() -> 0,
				(BiFunction<Integer, SynchronousSink<Quote>, Integer>)(index, sync) -> {
					Quote updatedQuote = updateQuote(this.prices.get(index));
					sync.next(updatedQuote);
				...
				//mapping with tuple to return the first object
				.map(tuple -> tuple.getT1())
				//Because values are generated in batches,
				//we need to set the timestamp to update
				.map(quote -> {
					quote.setInstant(Instant.now());
					return quote;
				})
				.log("com.andrei.service.QuoteGeneratorServiceImpl");
	}
